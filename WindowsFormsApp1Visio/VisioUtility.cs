...
namespace WindowsFormsApp1Visio
{
    ...
    public sealed class Utility {
        ...
        [CLSCompliant(false)]
        public static int ProductId(IVShape productShape) {
            ...
            int productId = 100; // ProductData.InvalidProductId;

            Cell productIdCell = null;

            // Ignore shapes that do not have the product ID cell.
            if (productShape.get_CellExistsU(Strings.ProductIdCellName,
                (int)VisExistsFlags.visExistsAnywhere) != 0) {

                // Get the cell containing the product ID from the shape.
                productIdCell = productShape.get_CellsU(Strings.ProductIdCellName);

                // Since product IDs are integers, get the cell value as an
                // integer without casting or rounding.
                productId = productIdCell.get_ResultInt(VisUnitCodes.visNoCast, 0);
            }
